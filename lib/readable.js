...
class KinesisReadable extends Readable {
  ...
  constructor (client/*: Object */, streamName/*: string */, options = {}) {
    ...
    this.shards = new function() {
      ...
      return {
        ...
        // removeById: function (shardId/*: string */){
        //   if (shardId in iterators) {
        //     delete shards[iterators[shardId]];
        //     delete iterators[shardId];
        //   }
        // },
        update: function(oldIterator, newIterator){
          iterators[shards[oldIterator]] = newIterator;
          shards[newIterator] = shards[oldIterator];
          delete shards[oldIterator];
        },
        popByIterator: function(iterator/*: string */){
          let shardid = shards[iterator];
          delete shards[iterator];
          delete iterators[shardid];
          return shardid;
        }
      }
  ...
  getShardIterator (shardId/*: string */, options/*: Object */) {
    ...
    return this.client.getShardIterator(params).promise()
      .then((data) => {
        ...
        //store shardIterator<=>shardId relation to adapt resharding
  ...
  reshard(shardIterator/*: string */){
    const shardId = this.shards.popByIterator(shardIterator);
    //find shards with parentdID == shardId
    getShards()
    .then(allShards => {
      //we separate existing from new ones
      return allShards.filter(shard => shard.ParentShardId == shardId)
    })
    .then(newShards => {
      return newShards.map(shard => shard.ShardId)
    })
    .then(newShardIds => {
      /* DUPLICATE CODE*/
      const whitelist = ['ShardIteratorType', 'Timestamp', 'StartingSequenceNumber']
      const shardIteratorOptions = Object.keys(this.options)
        ...
        .reduce((result, key) => Object.assign(result, {[key]: this.options[key]}), {})
      /* DUPLICATE CODE*/
      const newShardIterators = newShardIds.map(shardId => this.getShardIterator(shardId, shardIteratorOptions))
      return Promise.all(newShardIterators)
    })
    .then((shardIterators) => {
      shardIterators.forEach((shardIterator) => this.readShard(shardIterator))
    })
